...
abstract class BaseContact extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->projectsRelatedByFkContactClientIdScheduledForDeletion !== null) {
				if (!$this->projectsRelatedByFkContactClientIdScheduledForDeletion->isEmpty()) {
					ProjectQuery::create()
						->filterByPrimaryKeys($this->projectsRelatedByFkContactClientIdScheduledForDeletion->getPrimaryKeys(false))
						->delete($con);
			...
			if ($this->projectsRelatedByFkContactEmployerIdScheduledForDeletion !== null) {
				if (!$this->projectsRelatedByFkContactEmployerIdScheduledForDeletion->isEmpty()) {
					ProjectQuery::create()
						->filterByPrimaryKeys($this->projectsRelatedByFkContactEmployerIdScheduledForDeletion->getPrimaryKeys(false))
						->delete($con);
	...
	public function initRelation($relationName)
	{
		if ('ProjectRelatedByFkContactClientId' == $relationName) {
			return $this->initProjectsRelatedByFkContactClientId();
		}
		if ('ProjectRelatedByFkContactEmployerId' == $relationName) {
			return $this->initProjectsRelatedByFkContactEmployerId();
		}
	}
	...
	{
		if(null === $this->collProjectsRelatedByFkContactClientId || null !== $criteria) {
	...
	{
		$this->projectsRelatedByFkContactClientIdScheduledForDeletion = $this->getProjectsRelatedByFkContactClientId(new Criteria(), $con)->diff($projectsRelatedByFkContactClientId);

		foreach ($projectsRelatedByFkContactClientId as $projectRelatedByFkContactClientId) {
			// Fix issue with collection modified by reference
			if ($projectRelatedByFkContactClientId->isNew()) {
	...
	public function addProjectRelatedByFkContactClientId(Project $l)
	{
		...
		if (!$this->collProjectsRelatedByFkContactClientId->contains($l)) { // only add it if the **same** object is not already associated
	...
	{
		if(null === $this->collProjectsRelatedByFkContactEmployerId || null !== $criteria) {
	...
	{
		$this->projectsRelatedByFkContactEmployerIdScheduledForDeletion = $this->getProjectsRelatedByFkContactEmployerId(new Criteria(), $con)->diff($projectsRelatedByFkContactEmployerId);

		foreach ($projectsRelatedByFkContactEmployerId as $projectRelatedByFkContactEmployerId) {
			// Fix issue with collection modified by reference
			if ($projectRelatedByFkContactEmployerId->isNew()) {
	...
	public function addProjectRelatedByFkContactEmployerId(Project $l)
	{
		...
		if (!$this->collProjectsRelatedByFkContactEmployerId->contains($l)) { // only add it if the **same** object is not already associated
